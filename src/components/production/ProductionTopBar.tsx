...
async function fetchProductionNotifications(limit: number = 10): Promise<ProductionNotification[]> {
  try {
    const token = localStorage.getItem('authToken');
    const res = await fetch(`${API_CONFIG.baseURL}/production/alerts?limit=${limit}`, {
      headers: { Authorization: `Bearer ${token || ''}` },
    });
    if (!res.ok) return getMockProdNotifications();
    const data = await res.json();
    const list = data.alerts ?? data.data ?? data ?? [];
    const arr = Array.isArray(list) ? list : [];
    if (arr.length === 0) return getMockProdNotifications();
    ...
    return getMockProdNotifications();
  }
}

function getMockProdNotifications(): ProductionNotification[] {
  return [
    { id: '1', title: 'Line C changeover complete', description: 'Ready for next batch', status: 'open', priority: 'medium', createdAt: new Date().toISOString() },
    { id: '2', title: 'Low stock: Packaging Film', description: 'Reorder point reached', status: 'open', priority: 'high', createdAt: new Date().toISOString() },
    { id: '3', title: 'QC hold: Batch #4421', description: 'Awaiting inspection', status: 'open', priority: 'high', createdAt: new Date().toISOString() },
  ];
}
...
export function ProductionTopBar({ setActiveTab, onOpenDowntime }: ProductionTopBarProps = {}) {
  ...
  useEffect(() => {
    ...
    fetchProductionNotifications(15)
      .then((list) => setNotifications(list.length > 0 ? list : getMockProdNotifications()))
      .catch(() => setNotifications(getMockProdNotifications()))
      .finally(() => setNotificationsLoading(false));
  }, [notificationsOpen]);

  useEffect(() => {
    fetchProductionNotifications(8)
      .then((list) => setNotifications(list.length > 0 ? list : getMockProdNotifications()))
      .catch(() => setNotifications(getMockProdNotifications()));
  }, []);
  ...
  const handleSearchResultClick = useCallback(
    (item: SearchItem) => {
      ...
      navigate(`/dashboard/production/${tab}${search}`, { replace: true });
    },
              ...
              {!notificationsLoading &&
                notifications.map((n) => (
                  <div key={n.id ?? n._id ?? n.title} className="p-3 border-b border-[#F5F5F5] last:border-0 hover:bg-[#FAFAFA]">
                    ...
                    {n.createdAt && (
                      <p className="text-[10px] text-[#9E9E9E] mt-1">{new Date(n.createdAt).toLocaleString()}</p>
                    )}
                  </div>
                ))}
            </div>

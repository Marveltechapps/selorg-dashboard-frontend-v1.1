...
interface AddZoneWizardProps {
  ...
  existingZones: Zone[]; // For reference/validation
}

const STEPS = ['Basics', 'Draw', 'Link Stores', 'Rules', 'Review'];

export function AddZoneWizard({ isOpen, onClose, onSave, existingZones }: AddZoneWizardProps) {
            ...
            {currentStep === 1 && (
                <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 border rounded-lg overflow-hidden relative min-h-[300px]">
                        <MapArea 
                            zones={existingZones} // Show existing zones for context
                            stores={MOCK_STORES}
                            onZoneClick={() => {}}
                            onStoreClick={() => {}}
                            isDrawing={true}

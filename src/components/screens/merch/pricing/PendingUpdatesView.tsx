  ...
  const loadUpdates = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Don't show error if we have localStorage fallback
        setUpdates([]);
      }
    } catch (error) {
      ...
      try {
        const stored = localStorage.getItem('pricing_pending_updates');
        if (stored) {
          const parsed = JSON.parse(stored);
          setUpdates(parsed.filter((u: any) => u.status === 'pending'));
        }
      } catch (e) {

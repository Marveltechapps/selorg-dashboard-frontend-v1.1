...
export async function getApprovalSummary(date?: string): Promise<ApprovalSummary> {
  ...
  return {
    ...result,
    date: transformTimestamp(result.date).split('T')[0], // Extract date part
  };
}

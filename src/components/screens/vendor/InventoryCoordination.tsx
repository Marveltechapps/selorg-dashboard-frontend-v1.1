...
export function InventoryCoordination() {
  ...
  // component state (persistent to localStorage) - initialized from mock data
  const [stockItems, setStockItems] = useState<StockItem[]>(() => {
    try {
      const saved = localStorage.getItem('inventoryCoordination.stockItems');
      return saved ? JSON.parse(saved) : mockStockItems;
    } catch {
  ...
  const [agingAlerts, setAgingAlerts] = useState<AgingAlert[]>(() => {
    try {
      const saved = localStorage.getItem('inventoryCoordination.agingAlerts');
      return saved ? JSON.parse(saved) : mockAgingAlerts;
    } catch {
      return mockAgingAlerts;
    }
  });
  const [agingInventory, setAgingInventory] = useState<AgingInventory[]>(() => {
    try {
      const saved = localStorage.getItem('inventoryCoordination.agingInventory');
      return saved ? JSON.parse(saved) : mockAgingInventory;
    } catch {
      return mockAgingInventory;
    }
  });
  const [stockouts, setStockouts] = useState<Stockout[]>(() => {
    try {
      const saved = localStorage.getItem('inventoryCoordination.stockouts');
      return saved ? JSON.parse(saved) : mockStockouts;
    } catch {
      return mockStockouts;
    }
  });
  const [kpis, setKpis] = useState<KPI[]>(() => {
    try {
      const saved = localStorage.getItem('inventoryCoordination.kpis');
  ...
  const [vendorId, setVendorId] = useState<string | null>(() => {
    try {
      return localStorage.getItem('selectedVendorId');
    } catch {
      return null;
    }
  });
    ...
    (async () => {
      try {
        ...
        const job = await vendorInventoryApi.triggerInventorySync(vendorId, {
          source: 'ui_manual',
          initiatedBy: localStorage.getItem('auth_user') || 'ui',
        });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('inventoryCoordination.stockItems', JSON.stringify(stockItems));
      localStorage.setItem('inventoryCoordination.agingAlerts', JSON.stringify(agingAlerts));
      localStorage.setItem('inventoryCoordination.agingInventory', JSON.stringify(agingInventory));
      localStorage.setItem('inventoryCoordination.stockouts', JSON.stringify(stockouts));
      localStorage.setItem('inventoryCoordination.kpis', JSON.stringify(kpis));
    } catch (e) {
      // ignore storage errors in demo
    }
  }, [stockItems, agingAlerts, agingInventory, stockouts, kpis]);

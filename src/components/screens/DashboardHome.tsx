            ...
            <div className="flex items-end gap-1 mt-4 h-8 w-full">
               {stats.lastHourData.length > 0 ? (
                 stats.lastHourData.map((h: number, i: number) => (
                   <div key={i} className="flex-1 bg-[#BBF7D0] rounded-t-sm" style={{ height: `${h}%` }} />
                 ))
               ) : (
                 [40, 60, 45, 75, 30, 55, 65, 80, 50].map((h, i) => (
                   <div key={i} className="flex-1 bg-[#BBF7D0] rounded-t-sm" style={{ height: `${h}%` }} />
                 ))
               )}
            </div>
               ...
               <tbody className="divide-y divide-[#F0F0F0]">
                 {isLoading ? (
                   ...
                   liveOrders.map((order: any) => {
                     ...
                     return (
                       <tr key={order.order_id} className="hover:bg-[#FAFAFA] group cursor-pointer">

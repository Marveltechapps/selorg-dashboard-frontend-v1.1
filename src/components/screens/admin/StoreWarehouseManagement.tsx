...
export function StoreWarehouseManagement() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [storesData, warehousesData, staffData, zonesData, perfData, statsData] = await Promise.all([
        fetchStores(),
        fetchWarehouses(),
        fetchStaff(),
        fetchDeliveryZones(),
      ...
      if (!stores.length) setStores([]);
      if (!warehouses.length) setWarehouses([]);
      if (!staff.length) setStaff([]);

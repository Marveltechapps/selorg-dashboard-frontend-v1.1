...
export async function fetchOverlapWarnings(): Promise<OverlapWarning[]> {
  try {
    const response = await apiRequest<{ success: boolean; data: OverlapWarning[] }>('/merch/geofence/overlaps');
    return response.data || MOCK_OVERLAPS;
  } catch (error) {
    console.error('Failed to fetch overlap warnings:', error);
...
export async function createZone(zone: Partial<GeofenceZone>): Promise<GeofenceZone> {
  try {
    ...
    // Return mock zone as fallback
    const mockZone: GeofenceZone = {
      id: `zone-${Date.now()}`,
      ...
      center: zone.center || { lat: 19.0760, lng: 72.8777 },
      settings: {
        deliveryFee: 39,
        minOrderValue: 149,
        maxDeliveryRadius: 5,
...
export async function updateZone(zoneId: string, updates: Partial<GeofenceZone>): Promise<GeofenceZone> {
  try {
    const response = await apiRequest<{ success: boolean; data: GeofenceZone }>(`/merch/geofence/zones/${zoneId}`, {
      method: 'PUT',
      body: JSON.stringify(updates),
    });
    if (response && response.data) {
      // Update local mock data
      const zoneIndex = MOCK_ZONES.findIndex(z => z.id === zoneId);
      if (zoneIndex !== -1) {
        MOCK_ZONES[zoneIndex] = response.data;
      }
      return response.data;
    }
    throw new Error('No data returned');
  } catch (error: any) {
    console.error('Failed to update zone via API, using mock data:', error);
    // Update local mock data
    const zoneIndex = MOCK_ZONES.findIndex(z => z.id === zoneId);
    if (zoneIndex !== -1) {
      MOCK_ZONES[zoneIndex] = { ...MOCK_ZONES[zoneIndex], ...updates, updatedAt: new Date().toISOString() };
      return MOCK_ZONES[zoneIndex];
    }
...
export async function deleteZone(zoneId: string): Promise<void> {
  try {
    ...
    const index = MOCK_ZONES.findIndex(z => z.id === zoneId);
    if (index !== -1) {
      MOCK_ZONES.splice(index, 1);
    }
  } catch (error: any) {
    console.error('Failed to delete zone via API, removing from mock data:', error);
...
export async function toggleZoneStatus(zoneId: string, status: 'active' | 'inactive'): Promise<void> {
  try {
    ...
    const index = MOCK_ZONES.findIndex(z => z.id === zoneId);
    if (index !== -1) {
      MOCK_ZONES[index] = { ...MOCK_ZONES[index], status };
    }
  } catch (error: any) {
    console.error('Failed to toggle zone status via API, using mock data:', error);
    // Update mock data
    const index = MOCK_ZONES.findIndex(z => z.id === zoneId);
    if (index !== -1) {
      MOCK_ZONES[index] = { ...MOCK_ZONES[index], status };
...
export async function cloneZone(zoneId: string, newName: string): Promise<GeofenceZone> {
  try {
    ...
    console.error('Failed to clone zone via API, using mock data:', error);
    // Find zone in mock data
    const originalZone = MOCK_ZONES.find(z => z.id === zoneId);
    if (originalZone) {
      const cloned = { 
        ...originalZone, 
        id: `zone-${Date.now()}`,

...
export async function fetchCertifications(): Promise<Certification[]> {
  try {
    ...
    return response.data || [
      { id: 'CERT-001', name: 'ISO 27001:2022', issuer: 'BSI Group', certNumber: 'ISO-2024-001', type: 'ISO', status: 'active', issuedDate: '2024-02-10', expiryDate: '2027-02-10', scope: 'Information Security Management', auditedBy: 'BSI Group', nextAudit: '2025-02-10', score: 95, attachments: 3 },
      { id: 'CERT-002', name: 'PCI-DSS Level 1', issuer: 'PCI Security Standards Council', certNumber: 'PCI-2024-002', type: 'PCI-DSS', status: 'active', issuedDate: '2024-03-01', expiryDate: '2025-03-01', scope: 'Payment Card Data Security', auditedBy: 'Qualified Security Assessor', nextAudit: '2025-03-01', score: 98, attachments: 5 },
    ];
  } catch (error) {
...
export async function fetchAudits(): Promise<AuditRecord[]> {
  try {
    ...
    return response.data || [
      { id: 'AUD-001', name: 'Q1 2025 Internal Security Audit', type: 'internal', status: 'scheduled', scheduledDate: '2025-03-15T09:00:00Z', auditor: 'Sarah Chen', auditorOrg: 'Internal Audit Team', scope: ['Access Controls', 'Data Encryption'], findings: [], criticalIssues: 0, majorIssues: 0, minorIssues: 0 },
      { id: 'AUD-002', name: 'GDPR Compliance Review', type: 'external', status: 'completed', scheduledDate: '2024-11-01T09:00:00Z', completedDate: '2024-11-15T17:00:00Z', auditor: 'External Auditor', auditorOrg: 'Compliance Partners', scope: ['Data Protection', 'Privacy'], findings: [], criticalIssues: 1, majorIssues: 0, minorIssues: 1, overallScore: 92 },
    ];
  } catch (error) {
...
export async function fetchPolicies(): Promise<Policy[]> {
  try {
    ...
    return response.data || MOCK_POLICIES;
  } catch (error) {
...
export async function fetchMetrics(): Promise<ComplianceMetrics> {
  try {
    ...
    return response.data || MOCK_METRICS;
  } catch (error) {
...
export async function fetchViolations(): Promise<ViolationAlert[]> {
  try {
    ...
    return response.data || MOCK_VIOLATIONS;
  } catch (error) {
...
export async function uploadDocument(doc: Partial<ComplianceDocument> & { file?: File }): Promise<ComplianceDocument> {
  try {
    ...
    const mockDoc: ComplianceDocument = {
      ...
      fileSize: doc.file ? `${(doc.file.size / 1024 / 1024).toFixed(2)} MB` : '0 MB',
      version: '1.0',
      uploadedBy: 'current-user@quickcommerce.com',
...
export async function scheduleAudit(audit: Partial<AuditRecord>): Promise<AuditRecord> {
  try {
    ...
    const mockAudit: AuditRecord = {
      id: `audit-${Date.now()}`,
      name: audit.name || 'New Audit',
      type: audit.type || 'internal',
      status: 'scheduled',
      scheduledDate: audit.scheduledDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
      auditor: audit.auditor || '',
      auditorOrg: 'Internal Audit Team',
      scope: [],
      findings: [],
      criticalIssues: 0,
      majorIssues: 0,
...
<<<<<<< HEAD
  try {
    ...
    // Generate mock report and download
    const reportData = {

  ...
  const handleExportReport = () => {
    ...
    try {
      ...
      const reportData = [
        ...
        ['Key Metrics'],
        ['Metric', 'Value'],
        ['Avg Delivery Time', '14m 22s'],
        ['SLA Compliance', '94.2%'],
        ['Order Completion', '99.1%'],
        ['Customer Satisfaction', '4.8/5.0'],
        [''],
        ['Order Flow History'],
        ['Time', 'Orders/min'],
        ...analytics.orderFlowHistory.map(item => [item.time, item.orders.toString()]),
        [''],
        ['SLA Performance by Zone'],
        ['Zone', 'Actual (mins)', 'Target (mins)'],
        ...analytics.slaPerformanceByZone.map(item => [item.zone, item.actual.toString(), item.target.toString()]),
        [''],
        ['Rider Utilization'],
        ['Status', 'Count', 'Percentage'],
        ...analytics.riderUtilization.map(item => [item.status, item.count.toString(), `${item.percent}%`]),
      ];

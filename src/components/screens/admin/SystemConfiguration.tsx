...
export function SystemConfiguration() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const safeSet = async <T,>(fn: () => Promise<T>, setter: (v: T) => void) => {
        try {
          setter(await fn());
        } catch (_) {}
      };
      await safeSet(fetchGeneralSettings, setGeneralSettings);
      await safeSet(fetchDeliverySettings, setDeliverySettings);
      await safeSet(fetchPaymentGateways, setPaymentGateways);
      await safeSet(fetchNotificationSettings, setNotificationSettings);
      await safeSet(fetchTaxSettings, setTaxSettings);

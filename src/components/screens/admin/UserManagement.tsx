...
export function UserManagement() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const [usersData, rolesData, permsData, logsData, sessionsData] = await Promise.all([
        ...
        fetchAccessLogs().catch(err => {
          console.warn('Failed to fetch access logs:', err);
          return [];
        }),
        fetchLoginSessions().catch(err => {
      ...
      console.log('Data loaded:', { 
        ...
        permissions: permsData?.length || 0 
      });
      ...
      if (rolesData && rolesData.length > 0) {
        ...
        toast.warning('No roles found. Using fallback data.');
      }
    } catch (error: any) {
  ...
  console.log('UserManagement rendering with:', { 
          ...
          <div className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  const value = e.target.value;
                  console.log('Filter modal search changed:', value);
                  setSearchTerm(value);
                }}
              />
            </div>
            <div className="space-y-2">
              ...
              <Select 
                value={statusFilter} 
                onValueChange={(value) => {
                  console.log('Filter modal status changed:', value);
                  setStatusFilter(value);
                }}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select 
                value={roleFilter} 
                onValueChange={(value) => {
                  console.log('Filter modal role changed:', value);
                  setRoleFilter(value);
                }}
              >
                <SelectTrigger>

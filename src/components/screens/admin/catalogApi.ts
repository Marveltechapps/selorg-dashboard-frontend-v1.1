...
export async function createProduct(data: Partial<Product>): Promise<Product> {
  try {
    ...
    if (response.data) {
      // Add to mock products for immediate UI update
      MOCK_PRODUCTS.push(response.data);
      return response.data;
    }
...
export async function updateProduct(id: string, data: Partial<Product>): Promise<Product> {
  try {
    ...
    if (response.data) {
      // Update mock products array
      const index = MOCK_PRODUCTS.findIndex(p => p.id === id);
      if (index !== -1) {
        MOCK_PRODUCTS[index] = { ...MOCK_PRODUCTS[index], ...response.data };
      }
      return response.data;
    }
    // Fallback: update mock product
    const index = MOCK_PRODUCTS.findIndex(p => p.id === id);
    if (index !== -1) {
...
export async function deleteProduct(id: string): Promise<void> {
  try {
    ...
    // Remove from mock products array
    const index = MOCK_PRODUCTS.findIndex(p => p.id === id);
    if (index !== -1) {
...
export async function createCategory(data: Partial<Category>): Promise<Category> {
  try {
    ...
    // Always create mock category
    const mockCategory: Category = {
      id: `cat-${Date.now()}`,
      name: data.name || 'New Category',
      slug: data.slug || (data.name || 'new-category').toLowerCase().replace(/\s+/g, '-'),

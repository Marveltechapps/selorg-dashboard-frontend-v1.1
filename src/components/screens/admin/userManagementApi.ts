...
export const createUser = async (payload: CreateUserPayload): Promise<User> => {
  try {
    ...
    const userPayload = {
      ...payload,
      password: payload.password || `TempPass${Math.random().toString(36).slice(-8)}!`, // Generate random password
    };
    ...
    if (response.data) {
      // Add to mock users for immediate UI update
      MOCK_USERS.push(response.data);
      return response.data;
    }

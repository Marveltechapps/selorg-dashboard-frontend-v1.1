...
function FleetOverview() {
  ...
  const loadFleetData = async (isSilent = false) => {
    try {
      ...
      const response = await hsdApi.getFleetOverview({ storeId: 'DS-Brooklyn-04' });
      if (isMounted.current) {
  ...
  const handleDeviceControl = async (deviceId: string, action: 'lock' | 'reboot') => {
    try {
      ...
      await hsdApi.deviceControl(deviceId, { action, reason: `Manual ${action} from dashboard`, storeId: 'DS-Brooklyn-04' });
...
function LiveSessions() {
   ...
   const loadSessions = async (isSilent = false) => {
      try {
         if (!isSilent) setLoading(true);
         const data = await hsdApi.getLiveSessions({ storeId: 'DS-Brooklyn-04' });
         if (isMounted.current) {
            setSessions(data);
...
function IssueTracker() {
   ...
   const loadIssues = async (isSilent = false) => {
      try {
         if (!isSilent) setLoading(true);
         const data = await hsdApi.getIssues({ storeId: 'DS-Brooklyn-04' });
         if (isMounted.current) {
            setIssues(data);
         }
      } catch (error) {
